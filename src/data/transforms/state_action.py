...
from src.data.schema import DatasetMetadata, RotationType, StateActionMetadata
from src.data.transforms_base import InvertibleModalityTransform, ModalityTransform
import numpy as np
...
class Normalizer:
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.mode == "q99":
            ...
            if not (
                torch.all(normalized <= 1 + 1e-5) and torch.all(normalized >= -1 - 1e-5)
            ):
                print(
...
class PerHorizonNormalizer:
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.mode == "q99":
            ...
            mask = (
                (q01 == q99).unsqueeze(0).expand_as(x)
                if len(x.shape) > 2
                else (q01 == q99)
            )
            ...
            mask = (
                (std == 0).unsqueeze(0).expand_as(x) if len(x.shape) > 2 else (std == 0)
            )
            normalized = torch.where(mask, x, normalized)

        elif self.mode == "min_max":
            min_val = self.statistics["min"].to(dtype=x.dtype, device=x.device)[
                :horizon_len
            ]
            max_val = self.statistics["max"].to(dtype=x.dtype, device=x.device)[
                :horizon_len
            ]
            ...
            mask = (
                (min_val == max_val).unsqueeze(0).expand_as(x)
                if len(x.shape) > 2
            ...
            mask = (
                (abs_max == 0).unsqueeze(0).expand_as(x)
                if len(x.shape) > 2
                else (abs_max == 0)
            )
            normalized = torch.where(mask, torch.zeros_like(x), normalized)
    ...
    def _initialize_normalizers(self):
        ...
        for key in self.apply_to:
            ...
            if subkey not in self.per_horizon_statistics:
                print(
                    f"Warning: No per-horizon statistics found for {subkey}, skipping"
                )
                continue
