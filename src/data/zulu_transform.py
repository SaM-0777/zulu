...
from src.data.transforms_base import InvertibleModalityTransform
        ...
        if key == "text":
            ...
            for elem in features:
                ...
                try:
                    ...
                    if (
                        embodiment_tag_mapping is not None
            ...
            ids, mask = tokenizer.tokenize_instruction(
                output_values
            )
            batch[key] = ids
            ...
            ids, mask = tokenizer.tokenize_instruction(values)
            batch[key] = ids
            batch["text_attention_mask_negative"] = mask
        else:
            values = [elem[key] for elem in features]
            is_scalar = not hasattr(values[0], "shape")
            if is_scalar:
                #print(f"Key '{key}': Detected as SCALAR (Type: {type(values[0])}). Skipping stack/shape log.")
                batch[key] = torch.tensor(values) # Scalars don't need stacking
            else:
                # 2. It's a sequence/tensor, safely log the shapes
                shapes = [v.shape for v in values]
                if len(set(shapes)) > 1:
                    print(f"!!! CRASH RISK: Key '{key}' has mixed shapes. Fix required.")
...
class DefaultDataCollator(DataCollatorMixin):
    def __init__(
        self,
        tokenizer_path: str = "models/flan-t5-base",
        max_length: int = 200,
        num_views: int = 1,
        embodiment_tag_mapping=None,
    ):
        super().__init__()
        self.tokenizer = TextEncoder(
            dtype=torch.float32,
        )
    ...
    def __call__(
        ...
        batch = collate(
            features, self.tokenizer, self.num_views, self.embodiment_tag_mapping
        )
        return batch


class ZuluTransform(InvertibleModalityTransform):
    ...
    tokenizer_path: str = Field(
        default="models/flan-t5-base", description="Path to the tokenizer."
    )
    _tokenizer: Optional[TextEncoder] = PrivateAttr(default=None)

    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        # Initialize the tokenizer
        self._tokenizer = TextEncoder(dtype=torch.float32)
    ...
    def check_keys_and_batch_size(self, data):
        ...
        if "language" in grouped_keys:
            ...
            if len(language_keys) == 1:
                ...
                self._language_key = (
                    None  # Will be selected randomly in _prepare_language
                )
    ...
    def _prepare_language(self, data: dict):
        ...
        if self.formalize_language:
            formalized_language = formalize_language(raw_language)  # type: ignore
            return (
                formalized_language,
                is_lapa_instance,
                is_dream_instance,
    ...
    def _prepare_state(self, data: dict):
        ...
        state = data["state"]
        assert (
            state.shape[0] % self.state_horizon == 0
    ...
    def _prepare_action(self, data: dict):
        ...
        actions = data["action"]
        assert (
            actions.shape[0] % self.action_horizon == 0
    ...
    def apply_single(self, data: dict) -> dict:
        ...
        # doesn't matter as its not going to vision transformer
